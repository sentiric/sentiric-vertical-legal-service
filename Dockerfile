...
RUN CGO_ENABLED=0 GOOS=linux go build \
    ...
    -o /app/bin/sentiric-vertical-legal-service ./cmd/vertical-legal-service
...
COPY --from=builder /app/bin/sentiric-vertical-legal-service .
RUN chown appuser:appgroup ./sentiric-vertical-legal-service

# GÜVENLİK: Kullanıcıyı değiştir
USER appuser

ENTRYPOINT ["./sentiric-vertical-legal-service"]